...
template <typename T>
spiralCurveRet_t Full_Simple_Curve_Path(T world, MatrixXd matFinalPoint, VectorXd vecT, setting_t P, VectorXd vecMaxL)
{
	spiralCurveRet_t funcRet;
	...
	/// Control points of full path
	fullSpiralSegRet = Full_Simple_Segmentation(world, matFinalPoint, vecT, P, vecMaxL);  
...
template <typename T>
spiralSegRet_t Full_Simple_Segmentation(T world, MatrixXd matFinalPoint, VectorXd vecT, setting_t P, VectorXd vecMaxL)
{
	spiralSegRet_t funcRet;
	MatrixXd matXb, matYb, matZb;
	...
	Partial_Simple_Path_Segmentation(world, matFinalPoint, P, vecMaxL, matXb, matYb, matZb);
	...
	dx11s = matXb(matXb.rows()-1, matXb.cols()-1);
	dx22s = matFinalPoint(matFinalPoint.rows()-1, 0);
	dy11s = matYb(matYb.rows()-1, matYb.cols()-1);
	dy22s = matFinalPoint(matFinalPoint.rows()-1, 1);
	dz11s = matZb(matZb.rows()-1, matZb.cols()-1);
	dz22s = matFinalPoint(matFinalPoint.rows()-1, 2);
	...
	for (i=0; i<n-1; i++)
	{
		dXP1 = matXb(i, matXb.cols()-1);
		dXP2 = matXb(i+1, 0);
		dYP1 = matYb(i, matYb.cols()-1);
		dYP2 = matYb(i+1, 0);
		dZP1 = matZb(i, matZb.cols()-1);
		dZP2 = matZb(i+1, 0);
		
		if ( abs(matXb(i,matXb.cols()-1)-matXb(i+1,0)) > 1e-5 )
		{
			PathLineSegmentation(dXP1, dXP2, dYP1, dYP2, dZP1, dZP2, vecxs, vecys, veczs);
			
			matData.conservativeResize(matData.rows()+2, vecxs.rows()+vecys.rows()+veczs.rows());
			
			matData.block(matData.rows()-2, 0, 1, 4) = matXb.row(i);
			matData.block(matData.rows()-2, 4, 1, 4) = matYb.row(i);
			matData.block(matData.rows()-2, 8, 1, 4) = matZb.row(i);
			
			matData.block(matData.rows()-1, 0, 1, 4) = vecxs.transpose();
			matData.block(matData.rows()-1, 4, 1, 4) = vecys.transpose();
			matData.block(matData.rows()-1, 8, 1, 4) = veczs.transpose();
		}
		else
		{
			matData.conservativeResize(matData.rows()+1, 12);
			
			matData.block(matData.rows()-1, 0, 1, 4) = matXb.row(i);
			matData.block(matData.rows()-1, 4, 1, 4) = matYb.row(i);
			matData.block(matData.rows()-1, 8, 1, 4) = matZb.row(i);
		}
	}// end for
	...
	if (matData.rows() == 0 )
	{
		matFinData.resize(3, 12);
		
		matFinData.topRows(1) << vecX1s.transpose(), vecY1s.transpose(), vecZ1s.transpose();
		matFinData.bottomRows(2) << matXb.row(matXb.rows()-1), matYb.row(matYb.rows()-1), matZb.row(matZb.rows()-1),
							vecX1e.transpose(), vecY1e.transpose(), vecZ1e.transpose();
	}
	else
	{
		matFinData.resize(matData.rows()+3, 12);
		
		matFinData.topRows(1) << vecX1s.transpose(), vecY1s.transpose(), vecZ1s.transpose();
		matFinData.block(1, 0, matData.rows(), 12) = matData;
		matFinData.bottomRows(2) << matXb.row(matXb.rows()-1), matYb.row(matYb.rows()-1), matZb.row(matZb.rows()-1),
							vecX1e.transpose(), vecY1e.transpose(), vecZ1e.transpose();
	}
...
template <typename T>
void Partial_Simple_Path_Segmentation(T world, MatrixXd matPath, setting_t P, VectorXd vecMaxL,
							MatrixXd &matXb, MatrixXd &matYb, MatrixXd &matZb)
{
	VectorXd vecXP, vecYP, vecZP;
	VectorXd vecXb, vecYb, vecZb;
	...
	/// Iteration for calculate whold of the cubic bezier spiral
	matXb.resize(0,0);	matYb.resize(0,0);	matZb.resize(0,0);
	
	for (i=0; i<n; i++)
	{
		One_Simple_Curve_Segmentation_Smooth(world, vecXP.segment(i,3), vecYP.segment(i,3), vecZP.segment(i,3),
					 P, vecMaxL(i), vecXb, vecYb, vecZb );
		
		matXb.conservativeResize(matXb.rows()+1, vecXb.rows());
		matYb.conservativeResize(matYb.rows()+1, vecYb.rows());
		matZb.conservativeResize(matZb.rows()+1, vecZb.rows());
		
		matXb.row(i) = vecXb.transpose();
		matYb.row(i) = vecYb.transpose();
		matZb.row(i) = vecZb.transpose();
	}
}


template <typename T>
void One_Simple_Curve_Segmentation_Smooth(T world, VectorXd vecXP, VectorXd vecYP, VectorXd vecZP,
						setting_t P, double L, VectorXd &vecX, VectorXd &vecY, VectorXd &vecZ)
{
	VectorXd vecP1(3), vecP2(3), vecP3(3), vecTemp_a(3), vecTemp_b(3);
	VectorXd vecp1(3), vecp2(3), vecp3(3), vecp4(3);
	...
	// First calculation
	vecP1 << vecXP(0), vecYP(0), vecZP(0);
	vecP2 << vecXP(1), vecYP(1), vecZP(1);
	vecP3 << vecXP(2), vecYP(2), vecZP(2);
	
	vecTemp_a = vecP1 - vecP2;
	vecTemp_b = vecP3 - vecP2;
	
	
	/// Start from R and increase ///
	vecp1 = vecP2 + (vecTemp_a/vecTemp_a.norm())*2*P.d;
	vecp2 = vecP2 + (vecTemp_a/vecTemp_a.norm())*P.d;
	vecp3 = vecP2 + (vecTemp_b/vecTemp_b.norm())*P.d;
	vecp4 = vecP2 + (vecTemp_b/vecTemp_b.norm())*2*P.d;
	
	vecX << vecp1(0), vecp2(0), vecp3(0), vecp4(0);
	vecY << vecp1(1), vecp2(1), vecp3(1), vecp4(1);
	vecZ << vecp1(2), vecp2(2), vecp3(2), vecp4(2);
	
	
	/// more smooth path generation ///
	Ind = 1;	INC = 2;
	
	// calculation until collide
	while ( (CollisionSmooth(vecX, vecY, vecZ, world, P.SZR, P.SZH)==0) & ( Ind < L-1e-5) )
	{
		// condition
		if (Ind < L-INC)
			P.d = P.d+INC;
		else if ( (Ind > L-INC) & (Ind < L) )
			P.d = L;
		
		vecp1 = vecP2 + (vecTemp_a/vecTemp_a.norm())*P.d;
		vecp2 = vecP2 + (vecTemp_a/vecTemp_a.norm())*0.5*P.d;
		vecp3 = vecP2 + (vecTemp_b/vecTemp_b.norm())*0.5*P.d;
		vecp4 = vecP2 + (vecTemp_b/vecTemp_b.norm())*P.d;
		
		Ind = (vecP2-vecp1).norm();
